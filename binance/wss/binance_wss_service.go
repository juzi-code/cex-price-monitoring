...
func SubKLines(symbolIntervalPair map[string]string) {
	fmt.Println("Binance-准备订阅现货K线数据")
	wsKlineHandler := func(event *binanceconnector.WsKlineEvent) {
		...
		if math.Abs(priceChangePercent) > constant.PriceChangeThresholdMap[event.Kline.Interval] {
			...
			if quoteVolume24h >= constant.MinQuoteVolume24h {
				fmt.Println("现货-K线数据 ", binanceconnector.PrettyPrint(event))
